        ...
        return (
            ...
            >
                <div className={cn(
                    ...
                    "group-hover/card:-translate-y-6 group-hover/card:shadow-2xl group-hover/card:shadow-purple-500/40 group-hover/card:ring-2 group-hover/card:ring-purple-500 group-hover/card:scale-125"
                )}>
                    <Image
...
const ImageLightbox: React.FC<ImageLightboxProps> = ({
    ...
    return (
        ...
        >
            ...
            >
                <div className={cn("relative overflow-hidden rounded-[inherit] bg-neutral-900 border border-white/10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)]")}>
                    ...
                    <div
                        className={cn("px-8 py-7 bg-neutral-900 border-t border-white/5")}
                        style={{
                            opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
                    ...
                    >
                        <div className="flex items-center justify-between gap-6">
                                    ...
                                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/5 rounded-full border border-white/5">
                                        {projects.map((_, idx) => (
                                            <button
                                                ...
                                                className={cn("w-1.5 h-1.5 rounded-full transition-all duration-500", idx === internalIndex ? "bg-white scale-150" : "bg-white/20 hover:bg-white/40")}
                                            />
                                        ))}
                                    </div>
                            ...
                            <button className={cn("flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-white bg-purple-600 hover:bg-purple-500 rounded-xl shadow-lg shadow-purple-600/20 transition-all duration-300 hover:scale-105 active:scale-95")}>
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, projects, className, gradient }) => {
    ...
    const backBg = gradient || "linear-gradient(135deg, #262626 0%, #404040 100%)";
    const tabBg = gradient || "#404040";
    const frontBg = gradient || "linear-gradient(135deg, #404040 0%, #262626 100%)";

    return (
        <>
            <div
                className={cn("relative flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer bg-white/5 border border-white/10 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-2xl hover:shadow-purple-500/20 hover:border-purple-500/40 group", className)}
            ...
            >
                <div
                    ...
                    style={{ background: gradient ? `radial-gradient(circle at 50% 70%, ${gradient.match(/#[a-fA-F0-9]{3,6}/)?.[0] || 'var(--accent)'} 0%, transparent 70%)` : "radial-gradient(circle at 50% 70%, #8b5cf6 0%, transparent 70%)", opacity: isHovered ? 0.12 : 0 }}
                />
                ...
                <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-white/40 transition-all duration-500" style={{ opacity: isHovered ? 0.8 : 1 }}>{projects.length} {projects.length === 1 ? 'project' : 'projects'}</p>
                </div>
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-white/20 transition-all duration-500" style={{ opacity: isHovered ? 0 : 1, transform: isHovered ? "translateY(10px)" : "translateY(0)" }}>
                    <span>Hover</span>
                </div>
            </div>
